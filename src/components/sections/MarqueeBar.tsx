const ITEMS = [
  "Brand Identity", "Editorial Design", "Motion Graphics",
  "Packaging", "Web Design", "Art Direction", "Photography", "Visual Systems",
];

export function MarqueeBar() {
  const rep = [...ITEMS, ...ITEMS, ...ITEMS, ...ITEMS];
  return (
    <div className="overflow-hidden border-y border-[var(--border)] bg-[var(--surface)] py-3.5">
      <div className="flex whitespace-nowrap" style={{ animation: "marquee 34s linear infinite" }}>
        {rep.map((item, i) => (
          <span key={i} className="inline-flex items-center gap-6 px-7">
            <span className="label text-[var(--muted)]">{item}</span>
            <span className="text-[var(--coral)] text-xs leading-none">◆</span>
          </span>
        ))}
      </div>
    </div>
  );
}
